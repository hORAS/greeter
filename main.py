...
def noFaceMessageONCE():
    peopleDetected[:] = []
    peopleUnknown[:] = []
    displayMSG(grt.dayAndHour())
    ##CONTROL PARA QUE SE EJECUTE UNA VEZ, SIN INTERMITENCIA
    #OPCIONES: HORA. DíA DE LA SEMANA
    #FRASES GRACIOSAS
    #CONTADOR DE DíAS

# When the face is not identify
def createDefaultMessageONCE():
    peopleDetected[:] = []
    peopleUnknown.append('value')
    if len(peopleUnknown)==1:
        mesagge = grt.getUnkownMessage()
        displayMSG(mesagge)
    
# When the face is identify
def createPersonalizedMessageONCE(name):
    peopleUnknown[:] = []
    peopleDetected.append(name)
    if len(peopleDetected)==2:
        if peopleDetected[0]==peopleDetected[1]:
            mesagge = grt.chooseMessage(grt.getPersonIdx(name))
            displayMSG(mesagge)
    elif len(peopleDetected)>2:
        if peopleDetected[-2]!=peopleDetected[-2]:
            peopleDetected[:] = []
...
# When the face is identify
def createPersonalizedMessage(name):
    mesagge = grt.chooseMessage(grt.getPersonIdx(name))
    return mesagge

def noFaceMessage():
    peopleDetected[:] = []
    mesagge = grt.dayAndHour()
    return mesagge 
...
if __name__ == '__main__':
    ...
    parser.add_argument('model', type=str, help='Path to model protobuf (.pb) file', default='./model/model.pb')
    parser.add_argument('id_folder', type=str, nargs='+', help='Folder containing ID folders', default='./ids/')
